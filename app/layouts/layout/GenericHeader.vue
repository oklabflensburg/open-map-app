    ...
    <header class="generic-header">
        <div class="generic-header__content">
            ...
            <div class="generic-header__brand">
                <NuxtLink to="/" class="generic-header__logo">
                    <img src="/oklabflensburg_logo.svg" alt="Open Vacancy Map Logo" class="generic-header__logo-icon"
                        width="32" height="32" />
                </NuxtLink>

                <!-- Visitor Navigation -->
                <nav v-if="!isAdminPage" class="generic-header__nav">
                    <NuxtLink v-if="tokenStore.isAuthenticated" to="/admin/dashboard"
                        class="generic-header__nav-link" active-class="generic-header__nav-link--active">
                        {{ t('dashboard') }}
                    </NuxtLink>

                    <NuxtLink to="/" class="generic-header__nav-link"
                        :class="{ 'generic-header__nav-link--active': route.path === '/' }" exact>
                        {{ t('visitor_nav_map') }}
                    </NuxtLink>

                    <NuxtLink to="/contribute" class="generic-header__nav-link"
                        active-class="generic-header__nav-link--active">
                        {{ t('visitor_nav_contribute') }}
                    </NuxtLink>

                    <NuxtLink to="/about" class="generic-header__nav-link"
                        active-class="generic-header__nav-link--active">
                        {{ t('visitor_nav_about') }}
                    </NuxtLink>

                    <NuxtLink v-if="!tokenStore.isAuthenticated" to="/app/login" class="generic-header__nav-link"
                        active-class="generic-header__nav-link--active">
                        {{ t('visitor_nav_login') }}
                    </NuxtLink>
                </nav>
                ...
                <div v-if="tokenStore.isAuthenticated" class="generic-header__user-menu">
                    ...
                    <div v-if="isUserMenuOpen" class="generic-header__user-dropdown" @click="closeUserMenu">
                        ...
                        <nav class="generic-header__user-dropdown-nav">
                            <NuxtLink to="/admin/user/messages/inbox" class="generic-header__user-dropdown-item">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                                    <path
                            ...
                            <NuxtLink to="/admin/user/messages/send" class="generic-header__user-dropdown-item">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                            ...
                            <NuxtLink to="/admin/user/profile" class="generic-header__user-dropdown-item">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                                    <path
                            ...
                            <NuxtLink to="/admin/user/permissions" class="generic-header__user-dropdown-item">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
...
const supportedLocales = ['en', 'de', 'da'] as const
type LocaleCode = typeof supportedLocales[number]

const setDocumentLocale = (lang: LocaleCode) => {
...
// Theme and locale
const availableThemes: readonly ThemeName[] = ['light', 'dark'] as const
...
// Language switching
const setLanguage = async (lang: LocaleCode) => {
...
const initializePreferences = async () => {
    ...
    const storedLocale = localStorage.getItem('app-locale') as LocaleCode | null
    const localeToApply: LocaleCode = (storedLocale && supportedLocales.includes(storedLocale))
        ? storedLocale
        : locale.value as LocaleCode

    await setLocale(localeToApply)
    setDocumentLocale(localeToApply)
}
